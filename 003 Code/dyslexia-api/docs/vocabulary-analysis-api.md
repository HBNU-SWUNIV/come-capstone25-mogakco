...
- Body(JSON):
  - `textbookId` (number, preferred) or `documentId` (number)
...
- 검색 시 DB에 결과가 없으면 빈 배열을 반환합니다. FastAPI가 인라인 콜백으로 저장을 완료한 뒤, 클라이언트가 동일 요청을 재시도하면 저장된 결과를 받을 수 있습니다.
- S3 포인터/파일 기반 온디맨드 수집은 사용하지 않습니다.
...
어휘 분석이 “완전히 완료”되면, FastAPI는 최종 결과를 인라인(블록 배열)로 Spring에 전송하고, Spring은 이를 영속화(RDB 저장)합니다. S3 포인터 방식은 사용하지 않습니다.

## 콜백 타이밍과 흐름

1. FastAPI가 모든 블록의 어휘 분석을 완료
2. FastAPI → Spring 최종 콜백(인라인 블록 배열)
...
- Body(JSON)
  ...
  ```
FastAPI는 반드시 인라인 블록 배열을 전송해야 합니다.
